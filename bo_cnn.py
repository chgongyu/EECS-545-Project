...
def bo_tune_cnn(conv1_act,conv2_act,conv3_act,conv1_ker,conv2_ker,conv3_ker,conv1_filter,conv2_filter,conv3_filter,
    ...
    history = model.fit(train_images, train_labels, epochs=8, validation_split = 0.1, callbacks=[es])
...
cnn_bo.maximize(init_points=20, n_iter=40,acq='ucb')
end = time.time()
...
history = model.fit(train_images, train_labels, epochs=20, validation_split = 0.1, callbacks=[es])
...
print('Minimum accumulate loss :', np.minimum.accumulate(loss))
